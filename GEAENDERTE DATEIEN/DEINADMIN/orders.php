 ...
 * @version $Id: orders.php for Multisite 2019-09-30 19:47:51Z webchills $
 */
...
foreach ($orders_site as $multisite) {
  $orders_sites[] = array(
    'id' => $multisite['order_site'],
    'text' => $multisite['order_site'] . ' ');
    $orders_site_array[$multisite['order_site']] = $status['order_site'];
}
// eof Multi site
...
if ($oID) {
  ...
  $orders = $db->Execute("SELECT orders_id, order_site
                          FROM " . TABLE_ORDERS . "
        ...
        <div class="row noprint">
          <div class="form-inline">
            ...
            <div class="form-group col-md-2">
             ...
             <div class="form-group col-md-2">
                <?php
                echo zen_draw_form('multisite', FILENAME_ORDERS, '', 'get', '', true);
                echo zen_draw_label(HEADING_TITLE_STATUS, 'selectmultisite', 'class="sr-only"');
                echo zen_draw_pull_down_menu('multisite', array_merge(array(array('id' => '', 'text' => TEXT_ALL_ORDERS)), $orders_sites), (int)$_GET['multisite'], 'class="form-control" onChange="this.form.submit();" id="selectmultisite"');
                echo '</form>';
                ?>
            </div>
          </div>
...
//Multi site: add site_name
    ...
    if(isset($_GET['multisite'])&&($_GET['multisite']!='')){
      $search .= " and order_site='".zen_db_prepare_input($_GET['multisite'])."'";
    }
// eof Multi site
